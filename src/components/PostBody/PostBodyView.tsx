...
const PostBodyView = (props: Props): JSX.Element => {
  ...
  const intl = useIntl();
  const [searchFAB, setSearchFAB] = useState(true);
  const actionLinkRef = useRef(null);
  ...
  * {
    ...
    try {
      ...
      switch (type) {
        ...
        case 'markdown-external-link':
          ...
          console.log('external link clicked', href, actionLinkRef);
          actionLinkRef.current?.setModalVisible();
          break;
        case 'markdown-author-link':
          props.handleAuthorPress(author);
          break;
        case 'markdown-post-link':
          props.handlePostPress(author, permlink);
          break;
        case 'markdown-tag-link':
          props.handleTagPress(tag);
          break;
        ...
        case 'image':
          ...
          actionImageRef.current?.setModalVisible(true);
          break;
        default:
          break;
      }
  ...
  const _openExternalLInk = () => {
    ...
    // hide the modal
    actionLinkRef.current?.setModalVisible(false);
  };
  ...
  const _copyLinkToClipboard = (isImage: boolean) => {
    ...
    if (isImage) {
      link = selectedImage;
      actionImageRef.current?.setModalVisible(false);
    } else {
      link = selectedLink;
      actionLinkRef.current?.setModalVisible(false);
    }
  ...
  const _saveImage = async () => {
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        console.log('[_saveImage] uri', uri);
      }
  ...
  return (
    <Block flex>
      ...
      />
      <ActionSheet ref={actionLinkRef}>
        <Block center>
          <Button color="primary" onPress={_openExternalLInk}>
            {intl.formatMessage({id: 'Actionsheet.open'})}
          </Button>
          <Button color="warning" onPress={() => _copyLinkToClipboard(false)}>
      ...
      <ActionSheet ref={actionImageRef}>
        <Block center>
          <Button color="primary" onPress={() => _copyLinkToClipboard(true)}>
            {intl.formatMessage({id: 'Actionsheet.copy'})}
          </Button>
          <Button color="warning" onPress={_saveImage}>
